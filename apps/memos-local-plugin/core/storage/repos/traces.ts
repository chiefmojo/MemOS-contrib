...
/**
 * Narrow projection used by the WP #272 gain-inference pass. Carries only the
...
export function makeTracesRepo(db: StorageDb) {
  ...
  return {
     ...
     * (WP #272): there is no window where V is refreshed but gain provenance
    ...
    /**
     * WP #272 — stamp a historical screening attempt on one trace. Writes
    ...
    /**
     * WP #272 Phase C — un-stamp one trace so the idempotent screening pass
    ...
    /**
     * WP #272 — bounded narrow-projection member read for a reward-pass set.
    ...
    /**
     * WP #272 — narrow-projection, keyset-paged read of an episode's traces.
    ...
    /**
     * WP #272 — every trace id stamped by a given inference version. Feeds the
